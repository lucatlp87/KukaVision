...
void MainLoop::PrintUsage(const char* progName)
{
...
void MainLoop::TrainingStage()
{
    ...
    while(true)
    {
      ...
      if (insert_object.compare ("y") == 0)
      {
        ...
        cloud_by_kinect.LoadCloud("scene_bad.pcd");
...
void MainLoop::NewSceneStage()
{
  ...
  scene_object.SetSceneObjectVector(extracted_clusters);
    ...
    for (int cluster_idx = 0; cluster_idx < extracted_clusters.size(); ++cluster_idx)
    {
      ...
      for (boost::filesystem::directory_iterator it (DB_base_dir); it != boost::filesystem::directory_iterator (); ++it)
      {
        if (boost::filesystem::is_directory (it->status ()))
        {
          ...
          {
            scene_object.SetSubfolderList(search_path);
            corr_found = 1;
            break;
          }
        }
...
void MainLoop::KukaVisionStage()
{
	std::cout << "KukaVisionStage" << std::endl;
}