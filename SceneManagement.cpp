...
void
SceneManagement::VisualizeRefScene(std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> cluster_vector)
{
    ...
    int viewport = 1;
    // Viewports size
    double viewport_step = 1/(double)cluster_vector.size();
    ...
    for (double i = 0; i < ref_objects_list.size(); ++i)
    {
		...
		pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> cluster_color (cluster_vector[i], color_vector[1], color_vector[2], color_vector[3]);

		scene_viewer->createViewPort(i*viewport_step,0,(i+1)*viewport_step,viewport_step,viewport);
		scene_viewer->addPointCloud<pcl::PointXYZ> (cluster_vector[i], cluster_color, object_name.str().append("_info"), viewport);
	    ...
	    scene_viewer->addPointCloud<pcl::PointXYZ> (cluster_vector[i],cluster_color,object_name.str().append("_main"), 1);
