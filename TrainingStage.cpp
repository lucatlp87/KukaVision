...
TrainingStage::RunStage()
{
    ...
    pcl::console::print_error("#######################################################################################");
	pcl::console::print_error("#######################################################################################\n");
    std::cout << "Please, place only one object in the center of the scene and repeat the TRAINING"
    ...
    if (number_of_clusters > 0)
    {
       	...
       	for (std::vector<pcl::PointIndices>::const_iterator it = clusters_indices_vector.begin (); it != clusters_indices_vector.end (); ++it)
        {
            ...
            else
            {
                ...
                updated_dir = processing_object.SaveCloud(current_cluster_normals,current_signature);
