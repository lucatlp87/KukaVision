...
void
Object::SetObjectRefPose(Eigen::Matrix4f matrix_to_set)
{
	...
	// 	}
	object_ref_pose = matrix_to_set;
}
// SETTING OBJECT POSE IN ACTUAL SCENE
void
Object::SetObjectActualPose(Eigen::Matrix4f matrix_to_set)
{
	for(size_t i = 0; i < 4; ++i)
		for(size_t j = 0; j < 4; ++j)
...
Object::SetObjectPoseTransformation(bool in_out) 
{
	if (!in_out)
	{
		...
		object_pose_transformation = object_actual_pose.inverse()*object_ref_pose;
	}
	else
	{
		// Final pose matrix
		Eigen::Matrix4f out_of_the_table_pose;
