...
void
SceneManagement::UpdateInnerObjectsVector(int index_to_search, Eigen::Matrix4f pose, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_to_set)
{
...
void
SceneManagement::UpdateOuterObjectsVector(int index_to_search, Eigen::Matrix4f pose, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_to_set)
{
	// Local object
	Object outer_object;
	...
	outer_object.SetObjectType("Unknown");
	outer_object.SetObjectModelPath("Unknown");
...
{
	// Local matrix
	Eigen::Matrix4f matrix_to_print;
	...
	if (out_objects_list.size() > 0)
		for (int out_idx = 0; out_idx < out_objects_list.size(); ++out_idx)
		{
			...
			std::cout << "\n\t\tOBJECT " << out_idx+1 << " --------------------------------------------------------------------------------------------------------------------" << std::endl;
			// Object type
			std::cout << "\t\tType -> " << out_objects_list[out_idx].GetObjectType() << std::endl << std::endl;
			// Reference pose
